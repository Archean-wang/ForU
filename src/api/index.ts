...
async function startPlayback(context_uri: string, offset: number | string=0,
    device_id: string|null=null, position_ms: number=0) {
...
async function playTracks(uris:string[], offset:string|number=0, 
    device_id: string|null=null, position_ms: number=0) {
        const url = device_id ? `/me/player/play?device_id=${device_id}` : "/me/player/play";
        let off = typeof offset === "number" ? {position: offset} : {uri: offset}
        return await http.put(url, {
...
async function checkPlaylist(pid: string, ids: string):Promise<boolean[]> {
    return await http.get(`/playlists/${pid}/followers/contains`, {params: {ids }});
}
...
async function checkArtists(aids: string):Promise<boolean[]> {
    return await http.get(`/me/following/contains`, {params: {ids: aids, type: "artist" }});
}
...
async function unfollowArtists(aids: string) {
    return await http.delete(`/me/following`, {params: {ids: aids, type: "artist" }});
}
...
async function getArtistTop(aid: string, country:string="HK") {
    return await http.get(`/artists/${aid}/top-tracks`, {params: {market: country}})
}
...
async function removeItemsFromPlaylist(pid:string, uris:string[], snapshot_id: string) {
    ...
    {data: {tracks: uris.map(v=>{return {uri: v}}), snapshot_id}});
}
